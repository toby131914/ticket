...
function handleMessage(message, sendResponse) {
  ...
  if (message.action === 'precheckLogin') {
    precheckLogin(message.url, sendResponse);
    return;
  }
...
function precheckLogin(url, sendResponse) {
  if (!url || !/^https?:\/\//i.test(url)) {
    sendResponse({ success: false, message: '請先輸入表單網址' });
    return;
  }

  setConfig({ status: 'opening', message: '正在檢查 Google 登入狀態' });

  chrome.tabs.create({ url, active: true }, (tab) => {
    if (!tab?.id) {
      setConfig({ status: 'error', message: '無法開啟登入檢查分頁' });
      sendResponse({ success: false, message: '無法開啟分頁' });
      return;
    }

    waitForTabComplete(tab.id, 20000).then((readyTab) => {
      const currentUrl = readyTab.url || '';
      if (isGoogleLoginUrl(currentUrl)) {
        setConfig({
          status: 'error',
          message: 'Google 要求重新登入，請先在剛開啟的分頁完成登入'
        });
        sendResponse({ success: false, loginRequired: true, url: currentUrl });
        return;
      }

      setConfig({
        status: 'done',
        message: '登入狀態正常，請回控制頁設定搶票時間'
      });
      sendResponse({ success: true, url: currentUrl });
    }).catch((err) => {
      setConfig({ status: 'error', message: `登入檢查失敗：${err.message}` });
...
function isGoogleLoginUrl(url) {
  return /^https:\/\/accounts\.google\.com\//i.test(url)
    || /^https:\/\/myaccount\.google\.com\//i.test(url);
}
